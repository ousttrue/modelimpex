...
class Bone:
    def __init__(
        self, name_ja: str, name_en: str, position: Float3, parent_index: int
    ) -> None:
    ...
    def __init__(self, data: bytes) -> None:
        ...
        for _, v in enumerate(self.vertices):
            ...
            match flag:
                ...
                case _:
                    raise ValueError("unknown deform")

            self.deform_bones[v.bone.x] = self.deform_bones.get(v.bone.x, 0) + 1
            self.deform_bones[v.bone.y] = self.deform_bones.get(v.bone.y, 0) + 1
            self.deform_bones[v.bone.z] = self.deform_bones.get(v.bone.z, 0) + 1
            self.deform_bones[v.bone.w] = self.deform_bones.get(v.bone.w, 0) + 1

            _edge_scale = r.float32()

        # indices
        index_count = r.uint32()
        array_cls = index_type * index_count
        self.indices = r.array(array_cls)
        # textures
        ...
        for _i in range(material_count):
            material_name_ja = text_buf()
            material_name_en = text_buf()
            _material = r.struct(Material)
            _texture = texture_index()
            _sphere_texture = texture_index()
            _sphere_mode = r.uint8()
            toon_flag = r.uint8()
            if toon_flag == 0:
                _toon_index = texture_index()
            elif toon_flag == 1:
                _toon_index = r.uint8()
            _comment = text_buf()
            draw_count = r.uint32()
            self.submeshes.append(
                Submesh(material_name_ja, material_name_en, draw_count)
            )

        # bones
        self.bones: List[Bone] = []
        bone_count = r.uint32()
        for _i in range(bone_count):
            ...
            bone = Bone(
                bone_name_ja, bone_name_en, position * SCALING_FACTOR, parent_bone_index
            )
            self.bones.append(bone)

            _transform_layer = r.uint32()
            flags = r.uint16()

            if flags & BONE_HAS_TAIL:
                _tail_index = bone_index()
            else:
                bone.tail_position = r.struct(Float3) * SCALING_FACTOR  # type: ignore

            if flags & BONE_ROTATION_CONSTRAINT or flags & BONE_TRANSLATION_CONSTRAINT:
                _source_index = bone_index()
                _value = r.float32()

            if flags & BONE_ROLL_AXIS:
                _axis = r.struct(Float3)

            if flags & BONE_LOCAL_AXIS:
                _axis_x = r.struct(Float3)
                _axis_z = r.struct(Float3)

            if flags & BONE_EXTERNAL_PARENT:
                _key = r.uint32()

            if flags & BONE_HAS_IK:
                _effector_index = bone_index()
                _loop_count = r.uint32()
                _rotation_limit = r.float32()
                chain_length = r.uint32()
                for _j in range(chain_length):
                    _joint_index = bone_index()
                    joint_rotation_limit = r.uint8()
                    if joint_rotation_limit:
                        _min_limit = r.struct(Float3)
                        _max_limit = r.struct(Float3)
