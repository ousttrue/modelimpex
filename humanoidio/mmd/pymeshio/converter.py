...
from . import common
from .pmx import pmx_format as pmx
...
def pmd_to_pmx(src: pmd.Pmd) -> pmx.Pmx:
    """
    ...
    # model info
    dst.name = src.name
    dst.english_name = src.english_name
    dst.comment = src.comment.replace("\n", "\r\n")
    dst.english_comment = src.english_comment.replace("\n", "\r\n")

    # vertices
    def createDeform(bone0: int, bone1: int, weight0: float) -> pmx.Bdef1 | pmx.Bdef2:
        if weight0 == 0:
    ...
    def get_sphere_texture_file(path: str):
        if len(path) == 0:
            return None
        elif path.find("*") == -1:
            return None
    ...
    def get_texture_index(path: str) -> int:
        try:
            return texture_map[get_texture_file(path)]
        except KeyError:
            return -1

    def get_sphere_texture_index(path: str) -> int:
        try:
            return texture_map[get_sphere_texture_file(path)]
        except KeyError:
            return -1

    def get_sphere_texture_flag(path: str):
    ...
    def get_toon_shared_flag(m: pmd.Material):
        return 1

    def get_toon_index(m: pmd.Material):
        return m.toon_index
    ...
    def is_connected(b):
        if isinstance(b, model.Bone_Rolling):
            return False
        if isinstance(b, model.Bone_Tweak):
            return False
        return True

    def is_rotatable(b):
        if isinstance(b, model.Bone_Rotate):
            return True
        if isinstance(b, model.Bone_RotateMove):
            return True
        if isinstance(b, model.Bone_RotateInfl):
            return True
        if isinstance(b, model.Bone_IKRotateInfl):
            return True
        if isinstance(b, model.Bone_Rolling):
            return True
        if isinstance(b, model.Bone_IKTarget):
            return True
        if isinstance(b, model.Bone_IK):
            return True
        if isinstance(b, model.Bone_Unvisible):
            return True
        if isinstance(b, model.Bone_Tweak):
            return True
    ...
    def get_ik_link(bone_index):
        ...
        if b.english_name.find(b"knee") == -1:
            return model.IkLink(bone_index, 0, common.Vector3(), common.Vector3())
        else:
            return model.IkLink(
                bone_index,
                1,
    ...
    def get_external_infl(b):
        ...
        elif isinstance(b, model.Bone_Tweak):
            return b.tail_index, b.ik_index * 0.01, 0
        elif isinstance(b, model.Bone_IK):
            return -1, 0, 1
        else:
            return -1, 0, 0
    ...
    if len(src.morphs) > 0:
        ...
        dst.morphs = [
            model.Morph(
                ...
                morph_type=1,
                offsets=[
                    model.VertexMorphOffset(base.indices[i], pos)
                    for i, pos in zip(m.indices, m.pos_list)
                ],
            )
            for i, m in enumerate(src.morphs)
            if m.name != b"base"
        ]
    ...
    # bone
    root_display_slot = model.DisplaySlot(u("Root"), u("Root"), 1)
    root_display_slot.references.append((0, 0))  # bone  # center
    bone_display_slots = [
        model.DisplaySlot(
    ...
    for bone_index, group_index in src.bone_display_list:
        bone_display_slots[group_index - 1].references.append((0, bone_index))  # bone

    # exp
    exp_display_slot = model.DisplaySlot(u("表情"), u("Exp"), 1)
    ...
    exp_display_slot.references = [
        (1, i) for i, m in sorted(enumerate(src.morphs[1:]), key=morphOrder)  # exp
    ]
    ...
    dst.rigidbodies = [
        model.RigidBody(
            ...
            shape_position=(
                r.shape_position + src.bones[0].pos
                if r.bone_index == -1
...
def obj_to_pmx(obj_model, name, scale):
    ...
    def create_vertex(v):
        return model.Vertex(
            v[0], v[2] or common.Vector3(), v[1] or common.Vector3(), model.Bdef1(0), 0
        )
    ...
    dst.vertices = [
        create_vertex(obj_model.get_vertex(ref)) for ref in each_triangle(obj_model)
    ]
