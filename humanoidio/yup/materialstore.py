...
class MaterialStore:
    ...
    def add_texture(self, src: bpy.types.Image, buffer: BufferManager):
        ...
        dst = gltf.GLTFTexture(name=src.name, source=image_index, sampler=sampler_index)
        self.textures.append(dst)

    def get_material_index(
        self, material: bpy.types.Material, bufferManager: BufferManager
    ) -> int:
        ...
        if material:
            self._add_material(material, bufferManager)
        else:
            self.materials.append(gltf.create_default_material())
        return gltf_material_index

    def _add_material(self, src: bpy.types.Material, bufferManager: BufferManager):
        # texture
        color_texture = None
        normal_texture = None
